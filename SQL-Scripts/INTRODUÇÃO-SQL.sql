...
CREATE TABLE LIVROS(
    ...
    SEXO_DO_AUTOR CHAR(1) CHECK (SEXO_DO_AUTOR IN ('M','F')),
    NUMERO_DE_PAG INT,
    NOME_DA_EDITORA VARCHAR(20),
    VALOR_DO_LIVRO MONEY,
...
INSERT INTO LIVROS(NOME_DO_LIVRO, NOME_DO_AUTOR,SEXO_DO_AUTOR,NUMERO_DE_PAG,
NOME_DA_EDITORA,VALOR_DO_LIVRO,ESTADO_DA_EDITORA,ANO_PUBLICACAO)
VALUES ('SQL PARA LEIGOS','JOAO NUNES','M',450,'ADDISON',98.00,'SP','2018') ,
       ('RECEITAS CASEIRAS','CELIA TAVARES','F',210,'ATLAS',45.00,'RJ','2008'),
       ('PESSOAS EFEITVAS','EDUARDO SANTOS','M',390,'BETA',78.99,'RJ','2018'),
...
-- EX1 Trazer todos os dados.

SELECT * FROM LIVROS
GO

--EX2 Trazer o nome do livro e o nome da editora

SELECT NOME_DO_LIVRO , NOME_DA_EDITORA
FROM LIVROS
GO

-- EX3 Trazer o nome do livro e a UF dos livros publicados por autores do sexo masculino.
...
-- EX4 Trazer o nome do livro e o número de páginas dos livros publicados por autores do sexo
-- feminino.

SELECT NOME_DO_LIVRO, NUMERO_DE_PAG
FROM LIVROS
WHERE SEXO_DO_AUTOR = 'F'
GO

-- EX5 Trazer os valores dos livros das editoras de São Paulo.
SELECT VALOR_DO_LIVRO
FROM LIVROS
WHERE ESTADO_DA_EDITORA = 'SP'
GO

-- EX6 Trazer os dados dos autores do sexo masculino que tiveram livros publicados por São
-- Paulo ou Rio de Janeiro (Questão Desafio).

SELECT * FROM LIVROS
GO

SELECT NOME_DO_AUTOR , NOME_DO_LIVRO, NOME_DA_EDITORA, NUMERO_DE_PAG, VALOR_DO_LIVRO, ESTADO_DA_EDITORA, SEXO_DO_AUTOR, ANO_PUBLICACAO
FROM LIVROS
WHERE SEXO_DO_AUTOR = 'M' AND (ESTADO_DA_EDITORA = 'RJ' OR  ESTADO_DA_EDITORA = 'SP')
GO

CREATE VIEW V_AUXILIAR
AS
...
SELECT * FROM V_AUXILIAR
WHERE SEXO_DO_AUTOR = 'M' AND (ESTADO_DA_EDITORA = 'RJ' OR  ESTADO_DA_EDITORA = 'SP')
GO


SELECT COUNT(*) AS QUANTIDADE_DE_LIVROS
FROM LIVROS 
GO

SELECT SEXO_DO_AUTOR , COUNT(*) AS 'QUANTIDADE POR SEXO'
FROM LIVROS
...
SELECT F.departamento  AS 'DEPARTAMENTO', 
       COUNT(*) AS 'QUANTIDADE_DEPT_ROUPA_FILME'
FROM funcionarios F
WHERE F.departamento = 'ROUPAS' OR F.departamento = 'FILMES'
...
FROM funcionarios F
WHERE F.departamento = 'LAR' OR F.departamento = 'FILMES'
GROUP BY departamento

-- LAR TEM MAIS , VAI NA FRENTE DA VERIFICAÇÃO
...
-- IMPORTANDO ARQUIVO CSV PARA O BANCO
...
/*
BULK INSERT MAQUINAS
FROM 'D:\Daniel\Curso SQL - Mafra\LogMaquinas.csv'
WITH(
...
SELECT MAQUINA , FORMAT(AVG(QTD) , 'N2' , 'EN-US') AS 'MEDIA_QTD'
FROM MAQUINAS
GROUP BY MAQUINA
ORDER BY 2 DESC
GO
...
SELECT MAQUINA , FORMAT(QTD , 'N2' , 'EN-US') AS MEDIA , COUNT(*) AS MODA
FROM MAQUINAS
GROUP BY MAQUINA , QTD
ORDER BY 3 DESC
GO

/* QUAL A MODA DE CADA MAQUINA*/


-- MAQUINA 01
SELECT TOP 1 MAQUINA , QTD , COUNT(*) AS 'MODA'
FROM MAQUINAS
...
/* QUAL O MAXIMO E MINIMO , A AMPLITUDE DE CADA MAQUINA E A MEDIA  */

SELECT MAQUINA ,
       FORMAT(AVG(QTD) , 'N2' , 'EN-US') AS 'MEDIA',
       MAX(QTD) AS 'MAXIMO',
       MIN(QTD) AS 'MINIMO',
...
SELECT MAQUINA , FORMAT(STDEV(QTD) , 'N2') AS 'DESVIO PADRÃO'
FROM MAQUINAS
GROUP BY maquina
ORDER BY 1 ASC
GO
...
SELECT MAQUINA , FORMAT(VAR(QTD) , 'N2') AS 'VARIÂNCIA'
FROM MAQUINAS
GROUP BY MAQUINA
ORDER BY 1 ASC
GO
...
SELECT MAQUINA ,
       FORMAT(AVG(QTD), 'N2') AS 'MEDIA',
       FORMAT(MAX(QTD) , 'N2') AS 'MAXIMO',
       FORMAT(MIN(QTD) , 'N2') AS 'MINIMO',
       FORMAT(MAX(QTD) - MIN(QTD) , 'N2') AS 'AMPLITUDE',
       FORMAT(STDEV(QTD), 'N2') AS 'DESVIO PADRÃO',
       FORMAT(VAR(QTD), 'N2') AS 'VARIANCIA'
       FROM MAQUINAS
GROUP BY MAQUINA
ORDER BY 4 DESC
GO
...
/* MODA E MEDIANA */
WITH Frequencias AS (
    SELECT QTD, COUNT(*) AS Frequencia
    FROM MAQUINAS
    WHERE MAQUINA = 'Maquina 01'
    GROUP BY QTD
)
SELECT QTD, Frequencia
FROM Frequencias
WHERE Frequencia = (
    SELECT MAX(Frequencia) FROM Frequencias
);
GO


-- QUERY ESTATISTICA COMPLETA
SELECT  MAQUINA,
        COUNT(QTD) AS 'QUANTIDADE',
        FORMAT(SUM(QTD), 'N2') AS 'TOTAL',
        FORMAT(AVG(QTD) , 'N2') AS 'MEDIA',
        MAX(QTD) AS 'MAXIMO',
        MIN(QTD) AS 'MINIMO',
        FORMAT(MAX(QTD) - MIN(QTD) , 'N2') AS 'AMPLITUDE',
        FORMAT(VAR(QTD), 'N2') AS 'VARIANCIA',
        FORMAT(STDEV(QTD) , 'N2') AS 'DESVIO PADRÃO',
        FORMAT((STDEV(QTD) / AVG(QTD)) * 100.2 , 'N2') AS 'COEFICIENTE DE VARIAÇÃO'
        FROM MAQUINAS
GROUP BY MAQUINA
ORDER BY 1
GO
...
INSERT INTO LOCACAO VALUES('01/08/2019',23,3,100);
INSERT INTO LOCACAO VALUES('01/02/2019',56,1,400);
INSERT INTO LOCACAO VALUES('02/07/2019',23,3,400);
INSERT INTO LOCACAO VALUES('02/02/2019',43,1,500);
INSERT INTO LOCACAO VALUES('02/02/2019',23,2,100);
INSERT INTO LOCACAO VALUES('03/07/2019',76,3,700);
INSERT INTO LOCACAO VALUES('03/02/2019',45,1,700);
INSERT INTO LOCACAO VALUES('04/08/2019',89,3,100);
INSERT INTO LOCACAO VALUES('04/02/2019',23,3,800);
INSERT INTO LOCACAO VALUES('05/07/2019',23,3,500);
INSERT INTO LOCACAO VALUES('05/02/2019',38,3,800);
INSERT INTO LOCACAO VALUES('01/10/2019',56,1,100);
INSERT INTO LOCACAO VALUES('06/12/2019',23,3,400);
INSERT INTO LOCACAO VALUES('01/02/2019',56,2,300);
INSERT INTO LOCACAO VALUES('04/10/2019',76,3,300);
INSERT INTO LOCACAO VALUES('01/09/2019',32,2,400);
INSERT INTO LOCACAO VALUES('08/02/2019',89,3,100);
INSERT INTO LOCACAO VALUES('01/02/2019',23,1,200);
INSERT INTO LOCACAO VALUES('08/09/2019',45,3,300);
INSERT INTO LOCACAO VALUES('01/12/2019',89,1,400);
INSERT INTO LOCACAO VALUES('09/07/2019',23,3,1000);
INSERT INTO LOCACAO VALUES('01/12/2019',21,3,1000);
INSERT INTO LOCACAO VALUES('01/02/2019',34,2,100);
INSERT INTO LOCACAO VALUES('09/08/2019',67,1,1000);
INSERT INTO LOCACAO VALUES('01/02/2019',76,3,1000);
INSERT INTO LOCACAO VALUES('01/02/2019',66,3,200);
INSERT INTO LOCACAO VALUES('09/12/2019',90,1,400);
INSERT INTO LOCACAO VALUES('03/02/2019',23,3,100);
INSERT INTO LOCACAO VALUES('01/12/2019',65,5,1000);
INSERT INTO LOCACAO VALUES('03/08/2019',43,1,1000);
INSERT INTO LOCACAO VALUES('01/02/2019',27,31,200);
GO
...
SELECT F.NOME , 
       ...
       L.MIDIA
FROM GENERO G
INNER JOIN FILME F ON F.ID_GENERO = G.IDGENERO
INNER JOIN LOCACAO L ON L.ID_FILME = F.IDFILME
GO

-- CRIANDO UMA TABELA NOVA COM SELECT 
...
BEGIN
    ...
    -- Supondo que apenas uma linha é inserida
    SELECT 
        @IDLOCACAO = IDLOCACAO,
        @ID_FILME = ID_FILME,
        @DATA = DATA,
    ...
    -- Obter o nome do filme e o gênero
    SELECT 
        @NOME = F.NOME,
        @GENERO = G.GENERO
    FROM FILME F
    INNER JOIN GENERO G ON F.ID_GENERO = G.IDGENERO
    WHERE F.IDFILME = @ID_FILME;

    -- Executar a procedure com os parâmetros obtidos
    EXEC INSERE_DADOS @IDLOCACAO, @NOME, @GENERO, @DATA, @DIAS, @MIDIA;
END
GO

DROP TRIGGER TG_RELATORIO

CREATE PROCEDURE INSERE_DADOS (@IDLOCACAO INT,   @NOME VARCHAR(40),  @GENERO VARCHAR(30),   @DATA DATE,   @DIAS INT,  @MIDIA INT)
AS
        INSERT INTO RELATORIO_LOCADORA VALUES (@IDLOCACAO , @NOME , @GENERO, @DATA, @DIAS , @MIDIA )
GO
...
INSERT INTO LOCACAO VALUES (GETDATE(),22,7,200);
INSERT INTO LOCACAO VALUES (GETDATE(),20,3,300);
INSERT INTO LOCACAO VALUES (GETDATE(),10,2,100);
GO

USE EXERCICIO
GO

CREATE TRIGGER TG_DELETE_DADOS
ON DBO.LOCACAO
AFTER DELETE
AS
BEGIN
        ...
        SELECT @IDLOCACAO = IDLOCACAO FROM DELETED

        DELETE FROM RELATORIO_LOCADORA WHERE IDLOCACAO = @IDLOCACAO 
...
DELETE FROM LOCACAO WHERE IDLOCACAO = 36
GO
...
-- QUAL A MODA DOS SALÁRIOS . A MODA DOS SALÁRIOS DIZ ALGO RELEVANTE

SELECT F.salario , COUNT(*) AS 'MODA SALARIAL'
FROM funcionarios F
GROUP BY F.salario
ORDER BY 2 DESC
GO
...
SELECT F.departamento , COUNT(*) AS 'MODA DEPARTAMENTOS'
FROM funcionarios F
GROUP BY F.departamento
ORDER BY 2 DESC
GO
...
SELECT F.departamento , 
       FORMAT(STDEV(F.salario), 'N2') AS 'DP DO DEPARTAMENTO'
FROM funcionarios F
GROUP BY F.departamento
ORDER BY 2 DESC
GO

SELECT MAX(F.salario) - MIN(F.salario) AS 'AMPLITUDE DOS SALARIOS'
FROM funcionarios F
GO  

SELECT F.DEPARTAMENTO,
       COUNT(*) AS 'MODA DEPARTAMENTO',
       SUM(F.SALARIO) AS 'TOTAL',
       FORMAT(AVG(F.salario),'N2') AS 'MÉDIA SALARIAL',
       MAX(F.salario) AS 'MAIOR SALARIO',
       MIN(F.salario) AS 'MENOR SALARIO',
       MAX(F.salario) - MIN(F.salario) AS 'AMPLITUDE',
       FORMAT(STDEV(F.salario) , 'N2') AS 'DESVIO PADRÃO',
       FORMAT(VAR(F.salario) , 'N2') AS 'VARIÂNCIA',
       FORMAT(STDEV(F.salario) / AVG(F.salario) * 100 , 'N2') AS 'COEFICIENTE DE VARIAÇÃO'
FROM funcionarios F
GROUP BY departamento
ORDER BY 2 DESC
GO

-- VARIÁVEIS DUMMY PARA APLICAÇÃO DE MACHINE LEARNING

SELECT NOME , CARGO,
CASE
        WHEN CARGO = 'Financial Advisor' THEN 'Condição 01'
        WHEN CARGO = 'Sales Associate' THEN 'Condição 02'
...
-- FILTRAR BASEADOS EM MULTIPLAS COLUNAS E MULTIPLOS TIPOS -- CONSIDERAR AND E OR
...
-- FILTROS BASEADOS EM PADRÕES DE CARACTERES
...
-- USANDO O HAVING PARA FILTRAR OS SALARIOS MAIORES Q 400000

SELECT DEPARTAMENTO , SUM(SALARIO) AS 'TOTAL'
FROM funcionarios
WHERE departamento LIKE 'B%'
GROUP BY departamento
HAVING SUM(salario) > 4000000 -- USAR HAVING QUANDO USAR FUNÇÕES DE AGREGAÇÃO , SUM() , MAX() , COUNT()...
GO

/* FILTROS DE CONTADORES */

SELECT COUNT(*) AS 'FUNCIONÁRIOS TOTAIS' FROM funcionarios

/*
SELECT COUNT(*) AS 'FUNCIONÁRIOS TOTAIS',
...
SUM(CASE WHEN SALARIO > 140000 THEN 1 ELSE 0 END) AS 'SALARIOS MAIORES QUE 140K'
FROM funcionarios
GO
...
-- TAMANHO DA STRING
SELECT LEN('EXEMPLO') AS 'COMPRIMENTO'
GO
...
-- LOWER COLCOCA TODAS AS LETRAS EM MINÚSCULO 
SELECT DISTINCT LOWER(DEPARTAMENTO) FROM funcionarios
GO 

-- TRIM RETIRA OS ESPAÇOS IGUAL PYHTON
SELECT TRIM('      UNIDADOS      ') -- TRIM REMOVE ESPACOS
